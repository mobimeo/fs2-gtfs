...
object InterpreterSuite extends MutableIOSuite {
  ...
  test("a row access value should be evaluated to the row field value") {
    case (interpreter, row) =>
      IO.pure(expect(interpreter.eval(Ctx.empty, row, Value.Field(Value.Str("b"))) == Some("2")))
  }

  test("a row access value should return non if the row doesn't have the field") {
    case (interpreter, row) =>
      IO.pure(expect(interpreter.eval(Ctx.empty, row, Value.Field(Value.Str("d"))) == None))
  }

  test("a row access value should return non if the field cannot be computed") {
    case (interpreter, row) =>
      IO.pure(
  ...
  test("a context access value should be evaluated to the context value") {
    case (interpreter, row) =>
      IO.pure(
        expect(
          interpreter.eval(
  ...
  test("a context access value should be evaluated to None if it doesn't exist") {
    case (interpreter, row) =>
  ...
  test("a context access value should be evaluated to None if the key cannot be computed") {
    case (interpreter, row) =>
      val ctx = Ctx("a" := "1", "b" := Ctx("a" := "2"))
      IO.pure(
        expect(
